...
#                            }}
        ...
        fn = "run-1-results-%s.res" % LANG
        with open(fn, "a+") as f:
            for i, hit in enumerate(response["hits"]["hits"]):
                f.write(num+"\t0\t"+str(hit["_id"])+"\t"+str(i)+"\t"+
                        str(hit['_score'])+"\t"+"run-1"+"\n")
...
#    params = {"DFR", "DFI", "IB", "LMDirichlet", "LMJelinekMercer"}
    with open("qrels-%s_%s.txt" % (QUERIES, LANG)) as f_qrel:
...
#                    index=index)
            ...
            with open("run-1-results-%s.res" % LANG) as f_run:
...
#    reindex(client, "run-1-%s" % LANG, index)
